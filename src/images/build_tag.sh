...
if [ ${#args[@]} -eq 0 ]; then
    echo "Usage: PVA_VERSION=x.x.x ./build_tag.sh nlp-spacy-arm nlp-light-arm nlp-mosquitto-arm capture-speech-arm hotword-porcupine-arm hotword-snowboy-arm orchestrator-arm stt-google-arm stt-pocketsphinx-arm stt-wit-arm tts-mimic-arm"
    exit 0
fi
